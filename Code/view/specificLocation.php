    ...
    <div class="hero min-h-screen">
        <div class="hero-content text-center">
            <div class="max-w-screen">
                <h1 class="text-5xl font-bold"><?= htmlentities($item['name']) ?></h1>
                <div class="divider before:bg-neutral-50 after:bg-neutral-50"></div>
                <h2 class="text-xl"><?= htmlentities($item['place']) ?></h2>
                ...
                <div class="flex w-full md:items-center">
                        ...
                        <div class="carousel w-1/2 max-h-96">
                            ...
                            foreach ($images as $image) :
                                ...
                                <div id="slide<?= $imageCount ?>" class="carousel-item relative w-full">
                                    <img src="<?= htmlentities($image) ?>" class="object-contain w-full rounded-box"/>
                    ...
                    <?php else : ?>
                        <img src="<?= htmlentities($images[0]) ?>" class="object-contain w-1/2 rounded-box"/>
                    <?php endif; ?>
                    <div class="divider-horizontal"></div>
                    <div>
                        <p class="py-6 text-justify"><?= htmlentities($item['description']) ?></p>
                        <div class="divider before:bg-neutral-50 after:bg-neutral-50"></div>
                        <h2 class="text-xl text-left">Spécifications</h2>
                        <p class="py-6 text-left">Prix pour une nuit : <?= htmlentities($item['pricePerNight']) ?> CHF<br>
                            Nombre maximal de clients : <?= htmlentities($item['maximumNbOfClients']) ?><br>
                ...
                if (isset($_SESSION['userEmailAddress'])) :
                    ?>
                    <a href="index.php?action=booking&locationNumber=<?= htmlentities($item['locationNumber']) ?>">
